        ...
        <div className="fixed inset-0 z-50 flex md:hidden">
          ...
          <div className="relative bg-[#72253e] w-64 p-5 space-y-6 text-white animate-slide-in">
            ...
            <Link
              href="/"
              onClick={() => setIsOpen(false)}
              className={`block text-lg ${pathname === "/" ? "text-[#1CAAF6]" : ""
                }`}
            >
              ہوم پیج
            </Link>
            ...
            {sortedCategories.slice(0, 8).map((item) => {
              ...
              return (
                <Link
                  ...
                  href={`${item.href}?slug=${item.slug}`}
                  onClick={() => handleCategoryClick(item.id)}
                  className={`block text-lg ${isActive ? "text-[#1CAAF6]" : ""
                    }`}
                >
                  {item.name}
                </Link>
 ...
 useEffect(() => {
  categoriesApi.Banner().then((res) => {
    if (Array.isArray(res.data)) {
      const onlyBanners = res.data
        .filter(item => item.type === "Banner" && item.subType === "Image")
        .sort((a, b) => a.order - b.order);

      setBanners(onlyBanners);
      console.log("Final banners:", onlyBanners);
    }
  });
}, []);
  ...
  {banners.length > 0 && (
    <>
      {banners && (
        <img
          src={`${process.env.NEXT_PUBLIC_API_IMAGE_URL}${banners[0]?.url}`}
          alt={banners[0]?.title || "Banner 1"}
          className="rounded-[24px] w-full md:w-1/3 aspect-[16/9] object-cover cursor-pointer"
        />
      )}
      {banners[1] && (
        <img
          src={`${process.env.NEXT_PUBLIC_API_IMAGE_URL}${banners[1].url}`}
          alt={banners?.titl || "Banner 2"}
